/**
 * CodeForge Worker Bridge v4.0
 * ─────────────────────────────────────────────────────────────────────────────
 * Manages a pool of Web Workers and provides a clean Promise-based API.
 * 
 * ARCHITECTURE:
 *   - One persistent engine.worker.js for compute tasks
 *   - Promise-based request/response via unique IDs
 *   - Automatic reconnect if worker crashes
 *   - Pending request queue during worker restart
 *
 * USAGE:
 *   const suggestions = await WorkerBridge.suggest({ code, word, ext, path });
 *   const diagnostics = await WorkerBridge.lint({ code, ext });
 *   const parsed = await WorkerBridge.parseSnippet(bodyLines);
 */
...
const WorkerBridge = (() => {
  ...
  let _ready = false;
  const _queue = []; // messages queued before worker is ready
  ...
  function _createWorker() {
    try {
      ...
      try {
        _worker = new Worker('./js/workers/engine.worker.js');
      } catch (e2) {
        console.warn('[WorkerBridge] Web Workers unavailable, running sync fallback');
        _worker = null;
        return;
      }
    }
    ...
    _worker.onerror = (e) => {
      ...
      _pending.forEach(({ reject, timeout }) => {
        clearTimeout(timeout);
        reject(new Error('Worker crashed'));
      });
      _pending.clear();
      // Restart worker after short delay
      setTimeout(() => {
        _worker = null;
        _ready = false;
        _createWorker();
    ...
    _queue.forEach(msg => _send(msg.type, msg.payload, msg.resolve, msg.reject));
    _queue.length = 0;
  }

  function _send(type, payload, resolve, reject) {
    if (!_worker) {
      // Synchronous fallback (no workers)
      setTimeout(() => {
        try {
          resolve(_syncFallback(type, payload));
        } catch (e) {
          reject(e);
        }
      }, 0);
    ...
    if (!_ready) {
      _queue.push({ type, payload, resolve, reject });
      return;
    }
  ...
  function _request(type, payload) {
    return new Promise((resolve, reject) => {
      if (!_worker && !_queue.length) {
        _createWorker();
      }
      _send(type, payload, resolve, reject);
    });
  }

  // Minimal synchronous fallback when workers aren't available
  function _syncFallback(type, payload) {
    if (type === 'SUGGEST') {
      // Very basic keyword filter
      const { word } = payload;
      if (!word || word.length < 2) return [];
      return [];
    }
    if (type === 'LINT') return [];
    if (type === 'SNIPPET_PARSE') {
      const { body } = payload;
      return { text: body.join('\n').replace(/\$\{?\d+:?([^}]*)\}?/g, '$1').replace(/\$0/g, ''), tabStops: [] };
    }
    return null;
  }

  // Load worker source inline to avoid CORS issues in file:// contexts
  function _workerSource() {
    // Inlined worker source — works in file:// and CORS-restricted contexts
    return `/**
...
`;
  ...
  return {
    suggest(params) {
      return _request('SUGGEST', params);
    },
    lint(params) {
      return _request('LINT', params);
    },
    parseSnippet(body) {
      return _request('SNIPPET_PARSE', { body });
    },
    parseScope(params) {
      return _request('PARSE_SCOPE', params);
    },
    fuzzyRank(query, items) {
      return _request('FUZZY_RANK', { query, items });
    },
    isAvailable() {
      return _worker !== null;
    },
